...
export interface KeyboardOptions {
   ...
   * Or the value text changes when we typing on keyboard without click on 'accept'.
   * If We used with 'accept' button then We can declare text validation that start validate when We click on 'accept'
   * Otherwise (without 'accept' button) When We will making the validation process?       
   */
  isChangeValueTextAfterAcceptButton?: boolean;
  ...
  // 'isClosing' is true
  //We canceled this parameters because we declare them in KeyActions interface  
  //validate: (/* keyboard, value, isClosing */) => any | null; 
  //validate: ((args: string) => boolean) | null;
  //When We click on 'accept' keyboard button then We execute this method 
  //accept: ((args: string) => boolean | void) | null;
